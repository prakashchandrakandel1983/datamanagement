...
*Combined Graphs*
xtset country Year
xtline ilifelad // showing the trends in Ladder of Life in Various Countries but Maldives and Bhutan do not display trens as they do no t have data
...
//the combined graph shows the trends for Ladder of Life and Healthy Life Expectancy at birth. 
//The Ladder of Life trends fluctuate significantly in majority countries; however, the "Healthy Life Expectancy at birth" variable shows postitive trends in countries except for Bhutam and Maldives for whom data is missing.

****Histograms for various countries********
...
**Finding association between Ladder of Life and Social Support making**

graph twoway scatter isocsupp ilifelad, by(country)
gr export myfig.png, replace 
/* It appears that social support and lifeladder variables are positively coRrelated for Pakistan, India and Sri Lanka only.
*/ 

graph twoway connected ipov_ratio iLit_rate, sort // twoway graph between poverty ratio and Literacy Rate
// The relationship seems inverse as the literacy rate goes up, the poverty ration goes down. However, the graph shows that after the literacy percent exceeds one, the poverty ratio shoots up sifnificantly, which seems to be an anomaly to me.

graph dot ilifelad isocsupp ilifechoice igenerosity icorrpercep iposeffect inegaffect, over(ipov_ratio) //graph of Happiness Parameters over Poverty Ratio
//Here the meand of Ladder of Life appears to be the highest and the mean of generosity variable is the least.
...
// The average male population seems larger than average frmale population in the years from 2015 to 2020, and confidence interval seems wider for male population than for female population.
...
// The associaiton between poverty ratio and literacy rate seems positive for the people of age 15-24. It appears that the literacy rate for this group does not lead to decrease in poverty rate. However, the coefficient of literacy rate (6.35) does not seem statistically significant. 
...
local y "Nepal, India Bangladesh Pakistan" //These are the countries associated with the datasets.
disp "`y'"
...
summarize `myvars' //finding out the summary statistics of the Happiness-related variabels
...
reg ilifelad             iCPI_Value log_poptotal   ipov_ratio C2 C3 C4 C5 C6 C7 
//Many of the coefficients are removed due to collinearilty; Howver, the Ladder of life variable seems to be negatively assocaited with poverty ratio. For every unit rise in poverty rate leads to decline by 0.09 units for Ladder of Life variable.
...
`command' ilifelad iCPI_Value log_poptotal 	
}


// using forvalues command for running regrssion
forvalues country=1/7 {
    display "`country'"
    reg ilifelad iCPI_Value igenerosity
}
...
display "`x'"
reg ilifelad iCPI_Value log_poptotal if Year==`x'
}
...
//using forvalues for Happiness Paramenters to created nested loops
forval i = 2015/2020 {  
    forval j = 1/7 {
		display  "`i'"
        foreach v of var ilifelad isocsupp igenerosity {  
            summarize `v' if Year == `i' & country == `j'  
        }  
    }  
} 

//using graphs for regressions
reg ilifelad isocsupp //regressing life Ladder on Poverty Ratio
local r2: display %5.2f e(r2) 
twoway (lfitci ilifelad isocsupp ) (scatter ilifelad isocsupp), note(R-squared=`r2')
...
//Determining joint-non-missingness using nested foreachloops and branching using population parameters//
 ...
 foreach var1 of varlist popmale  {
    foreach var2 of varlist popfemale ipov_ratio ilifelad {
...
foreach var in  ilifelad iloggdppc isocsupp ilifexpbirth ilifechoice igenerosity {
...
drop if (regexm((SeriesName), "^W")) //the women decision making variable was dropped as it had too few data
